...
class App extends Component {
	...
	getPoints = () => {
		...
		axios
			.get(
				"https://ukce.danjscott.co.uk/api/game/player_points/" +
					this.state.gameId
			)
			.then((res) => {
	...
	changePlayer = () => {
		...
		axios.post(
			"https://ukce.danjscott.co.uk/api/game/active_player/" +
				this.state.gameId
		);
	};
	...
	showNextComponent = (playerOne, playerTwo) => {
		...
		axios.get("https://ukce.danjscott.co.uk/api/game").then((res) => {
			this.setState({
	...
	start = () => {
		let connection = new WebSocket(
			"ws://" + this.state.websocketServerIp + ":8765"
		);
	...
	async adjustScore(activePlayer, increment) {
		...
		var res = await axios.post(
			"https://ukce.danjscott.co.uk/api/game/manual_points/" +
				this.state.gameId,
			data
		);
		if (res.status === 200) {
			...
			<div className="App">
				<Container className="main">
					{!this.state.showScores && (
						<PlayerNameInput
							showNextComponent={this.showNextComponent}
						></PlayerNameInput>
					)}
					{this.state.showScores && (
						<div>
							<PlayerScore
								...
								active={this.state.playerOneActive}
								score={this.state.playerOnePoints}
							/>
							<Row>
								<Col xs={{ span: 4, offset: 2 }}>
									<Button
										onClick={() =>
											this.adjustScore("player_one", true)
										}
										color="warning"
										className="inc-btn"
									>
							...
							<PlayerScore
								...
								active={this.state.playerTwoActive}
								score={this.state.playerTwoPoints}
							/>
							<Row>
								<Col xs={{ span: 4, offset: 2 }}>
									<Button
										onClick={() =>
											this.adjustScore("player_two", true)
										}
										color="warning"
										className="inc-btn"
									>
